...
const migration = ["20260922120000_journalism_content_department_scope.sql", "20260922123000_journalism_department_code_alignment.sql", "20260923110000_journalism_scope_hardening.sql"].map((name) => fs.readFileSync(new URL(`../../supabase/migrations/${name}`, import.meta.url), "utf8")).join("\n");
const hardening = fs.readFileSync(new URL("../../supabase/migrations/20260923110000_journalism_scope_hardening.sql", import.meta.url), "utf8");
...
test("database enforces Content scope for every Journalism security-definer entry point", () => {
  ...
  assert.match(hardening, /v_role_code not in \('tong_bien_tap', 'pho_tong_bien_tap'\)/);
