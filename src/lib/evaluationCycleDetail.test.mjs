...
test("cycle rows link to a full result detail page", () => { assert.match(list, /evaluation-cycles\/\$\{cycle\.id\}/); assert.match(detail, /Tổng hồ sơ/); assert.match(detail, /Trưởng phòng/); assert.match(detail, /Tổng Biên Tập/); assert.match(detail, /Tổng điểm/); });
...
test("evaluation UI spells out Tổng Biên Tập", () => { assert.doesNotMatch(detail, />TBT<|Chờ TBT/); assert.match(detail, /Chờ Tổng Biên Tập/); });
