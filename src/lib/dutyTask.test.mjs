...
test("personal and organization duty schedules share one viewer filter", () => {
  ...
  assert.match(page, /raw\.scope === "personal"/);
...
test("duty review summary is a separate participant-only page", () => {
  const viewer=readFileSync("src/components/DutyScheduleViewer.tsx","utf8");const route=readFileSync("src/app/api/duty-schedule/summary/route.ts","utf8");const evidenceRoute=readFileSync("src/app/api/tasks/duty/review/route.ts","utf8");const page=readFileSync("src/app/duty-schedule/summary/page.tsx","utf8");const repository=readFileSync("src/lib/dutyTaskRepository.ts","utf8");
  const detail=readFileSync("src/app/duty-schedule/summary/[employeeId]/page.tsx","utf8");const table=readFileSync("src/components/DutySummaryTable.tsx","utf8");
  assert.match(viewer,/Tổng kết đánh giá/);assert.match(page,/DutySummaryTable/);assert.match(page,/isParticipant/);assert.match(page,/summary=restricted/);assert.match(route,/isParticipant/);assert.match(route,/role_code!=="admin"/);assert.match(route,/role_code==="admin"\?undefined:guard\.actor\.id/);assert.match(repository,/async isParticipant/);assert.match(repository,/async summary\(from: string, to: string, assigneeId\?: string\)/);assert.match(route,/apiError\("forbidden",403\)/);assert.match(table,/summary\/"\+r\.employeeId/);assert.match(detail,/employeeId!==user\.id/);assert.match(detail,/participantReviews/);assert.match(detail,/canViewEvidence/);assert.match(detail,/Xem minh chứng/);assert.match(evidenceRoute,/assignee_id !== guard\.actor\.id/);assert.doesNotMatch(route,/issue_notes|evidence_path|evidence_name/);assert.doesNotMatch(detail,/issue_notes|evidence_path/);
});

test("duty roster exposes exactly three positions", () => { const positions = readFileSync("src/lib/dutyRoster.mjs", "utf8"); for (const position of ["Xuất bản", "Biên tập", "Phóng viên"]) assert.match(positions, new RegExp(position)); assert.doesNotMatch(positions, /Biên tập bước [12]/); });
...
test("editor position is limited to the approved accounts", () => { const shell = readFileSync("src/components/DutyTaskShell.tsx", "utf8"); const sql = readFileSync("supabase/migrations/20260909170000_three_position_duty_roster.sql", "utf8"); for (const username of ["leson", "triduong", "thihung", "hongninh"]) assert.match(shell, new RegExp(username)); assert.match(sql, /v_position='Biên tập'/); });
