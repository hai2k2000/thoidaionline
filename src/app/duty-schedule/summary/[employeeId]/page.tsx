...
type Review={result:string;on_time:boolean;reviewed_at:string};type DetailRow={id:string;due_date:string;due_time:string|null;duty_position:string;departments:{name:string}|null;assignee:{full_name:string|null}|null;duty_task_reviews:Review[]|Review|null};
export default async function DutyParticipantReviewPage({params,searchParams}:Props){
 ...
 const rows=result.rows as unknown as DetailRow[];const name=rows[0]?.assignee?.full_name??"Nhân sự";
