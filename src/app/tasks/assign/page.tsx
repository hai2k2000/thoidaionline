...
export default async function TaskAssignPage({ searchParams }: Props) {
  ...
  if (rawParams.kind === "journalism") {
    redirect(canUseJournalism({ roleCode: user.role_code, departmentCode: user.department_code, rbacPermissions: user.rbacPermissions }) ? "/journalism/tasks/new" : "/tasks");
  }
