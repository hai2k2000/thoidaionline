...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const allowed = new Set(["workKindId", "plannedPublicationAt", "location", "editorialNotes", "expectedUpdatedAt"]);
  if (Object.keys(body).some((key) => !allowed.has(key))) return apiError("invalid_request", 400);
  const detail = await serverSupabase.from("journalism_task_details").select("publication_status,work_kind_id,planned_publication_at,location,editorial_notes,updated_at").eq("task_id", taskId).maybeSingle();
  ...
  const { data, error } = await serverSupabase.rpc("api_update_journalism_metadata_v1", {
    ...
    p_expected_updated_at: body.expectedUpdatedAt === undefined ? detail.data.updated_at : asIsoDate(body.expectedUpdatedAt),
  });
