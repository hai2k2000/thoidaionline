...
import { parseManualPublicationReport } from "@/lib/journalismManualPublicationValidation";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const parsed = parseManualPublicationReport(await readJsonObject(request));
  if (!parsed.ok) return apiError("invalid_request", 400);
  const { value } = parsed;
  const { data, error } = await serverSupabase.rpc("api_upsert_journalism_publication_report_v1", {
