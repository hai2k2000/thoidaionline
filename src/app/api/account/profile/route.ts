...
export async function PATCH(request: Request) {
  ...
  const body = await request.json().catch(() => null) as { fullName?: unknown; email?: unknown; phone?: unknown; preferences?: unknown } | null;
  ...
  const preferences = normalizeAccountPreferences(body?.preferences);
  const { error } = await serverSupabase.from("staff_users").update({ full_name: fullName, email, phone, preferences }).eq("id", actor.id).eq("active", true);
