...
export async function GET(request: Request) {
  ...
  let attendanceQuery = serverSupabase.from("attendance_logs").select("user_id,work_date,status");
  if (from && to) attendanceQuery = attendanceQuery.gte("work_date", from).lte("work_date", to);
  const [{ data: people, error: pe }, { data: tasks, error: te }, { data: attendance, error: ae }] = await Promise.all([
  ...
  // Attendance is an optional HR module; keep the evaluation summary usable
  // while its table/migration is unavailable.
  ...
  const rows = sortStaffRows((people ?? []).filter((p: any) => !((p.username ?? "").toLowerCase() === "admin") && !["tong_bien_tap", "tbt_read_only"].includes(Array.isArray(p.roles) ? p.roles[0]?.code : p.roles?.code)) as any).map((p: any) => {
    ...
    const employeeAttendance = ae ? [] : (attendance ?? []).filter((log: any) => log.user_id === p.id);
    const attended = employeeAttendance.filter((log: any) => ["present", "late"].includes((log.status ?? "").toLowerCase())).length;
    const attendanceRate = employeeAttendance.length ? Math.round(attended * 1000 / employeeAttendance.length) / 10 : null;
    return { id: p.id, name: p.full_name, department: p.departments?.name ?? "—", total: own.length, done: done.length, returned: returned.length, inProgress: inProgress.length, waiting: waiting.length, overdue: overdue.length, attendanceRate, score: scores.length ? scores.reduce((a: number, b: number) => a + b, 0) : null, tasks: own.map((t: any) => ({ id: t.id, title: t.title, status: t.status, dueDate: t.due_date, score: Array.isArray(t.completion_score) ? t.completion_score[0]?.total_score : null })) };
  });
