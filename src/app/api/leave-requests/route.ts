...
export async function POST(request: Request) {
  ...
  if (!DATE.test(startDate) || !DATE.test(endDate) || endDate < startDate || !PERIODS.has(startPeriod) || !PERIODS.has(endPeriod) || !TYPES.has(leaveType) || reason.length < 3 || reason.length > 1000) return apiError("invalid_request", 400);
