    ...
    <main className="min-h-screen bg-slate-50 p-6 text-slate-900">
      <div className="mx-auto max-w-7xl lg:grid lg:grid-cols-[260px_1fr] lg:gap-4">
        ...
        <section className="rounded-xl border bg-white p-4">
          ...
          <div className="table-scroll rounded-lg border border-slate-200" tabIndex={0} aria-label="Danh sách tài liệu, cuộn ngang để xem thêm">
            ...
            <tbody>
              {filteredRows.map((r) => (
                <tr key={r.id ?? r.doc_code} className="cursor-pointer" onClick={() => r.id && router.push(`/documents/${r.id}`)}>
                  ...
                  <td className="px-3 py-2">{directionLabel[r.direction ?? ""] ?? r.direction ?? "-"}</td>
                  ...
                  <td className="px-3 py-2"><span className={`table-status ${r.status === "done" ? "table-status-success" : r.status === "archived" ? "table-status-neutral" : r.status === "in_progress" ? "table-status-warning" : "table-status-neutral"}`}>{docStatusLabel[r.status ?? "new"] ?? (r.status ?? "Mới")}</span></td>
                </tr>
              ))}
