...
const statusLabel = (status: string, approvalRequired = false) => ({
  ...
  waiting: approvalRequired ? "Chờ duyệt giao việc" : "Chờ duyệt nhận việc",
  pending_review: approvalRequired ? "Chờ duyệt hoàn thành" : "Chờ chấm điểm",
...
export default function TaskDetailShell({ task, capabilities, userLabel, journalismWorkKinds, journalismWorkKindsLoadFailed, journalismStructureOptions }: {
  ...
  const reviewClaim = async (decision: "approve" | "reject") => {
    let reason: string | null = null;
    if (decision === "reject") {
      reason = window.prompt("Lý do trả lại yêu cầu nhận việc:");
      if (!reason?.trim()) { setMessage("Vui lòng nhập lý do."); return; }
    }
    await jsonPost("/api/tasks/claim/review", { taskId: task.id, decision, reason });
  };
      ...
      <main className="min-w-0 flex-1 space-y-2.5">
        <header className="sticky top-3 z-30 rounded-2xl bg-white/95 px-4 py-4 shadow-sm backdrop-blur sm:px-5">
          <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex max-w-full shrink-0 flex-nowrap items-center gap-1 overflow-x-auto pb-1 sm:justify-end">
              ...
              <span className="shrink-0 rounded-full bg-orange-100 px-3 py-1 text-sm font-semibold text-orange-800">{statusLabel(task.status, task.approval_required)}</span>
              {canComplete ? <button data-testid="task-completion-action" disabled={busy} onClick={() => jsonPost(`/api/tasks/${task.id}/${completePath}`)} className="shrink-0 whitespace-nowrap rounded-lg bg-emerald-700 px-2 py-1.5 text-xs font-semibold text-white disabled:opacity-50">{personal ? (task.approval_required ? "Gửi duyệt hoàn thành" : "Hoàn thành") : "Gửi kết quả hoàn thành"}</button> : null}
              {hasTaskActions ? <>
        ...
        <div className="grid items-start gap-3 lg:grid-cols-[minmax(0,1fr)_390px] xl:grid-cols-[minmax(0,1fr)_430px]">
          ...
          <aside aria-label="Thông tin nhanh" className="order-first space-y-2.5 lg:order-none lg:sticky lg:top-3">
            <Section title="Thông tin chung"><dl className="grid grid-cols-2 gap-1.5"><Item label="Loại" value={personal ? "Nhiệm vụ cá nhân" : "Công việc được giao"} /><Item label="Hình thức" value={task.assignment_source === "self_registered" ? "Tự đăng ký" : task.assignment_source === "leadership_assigned" ? "Lãnh đạo giao" : "Không xác định (dữ liệu cũ)"} /><Item label={task.assignment_source === "self_registered" && ["in_progress", "pending_review", "done"].includes(task.status) ? "Người duyệt giao việc" : "Người giao việc"} value={task.assignment_source === "self_registered" ? (["in_progress", "pending_review", "done"].includes(task.status) ? task.assignment_approver?.full_name ?? "—" : "—") : task.created_by_user?.full_name ?? "—"} /><Item label="Phụ trách" value={task.owner?.full_name ?? "—"} /><Item label="Người duyệt" value={task.reviewer?.full_name ?? "—"} /><Item label="Phòng ban" value={task.departments?.name ?? "—"} /><Item label="Hạn" value={dueText(task)} /></dl></Section>
