...
export default function TaskDetailShell({ task, capabilities, userLabel }: {
  ...
  const submitScoreAndApprove = async (event: FormEvent) => {
    ...
    await jsonPost(`/api/tasks/${task.id}/review-completion`, { requirementResults: requirementResults.map((achieved, index) => ({ index, achieved })), collaborationScore, initiativeScore, note: scoreNote });
  };
...
function ScoreField({ label, value, onChange }: { label: string; value: number; onChange: (value: number) => void }) { return <label className="grid gap-1 text-sm font-semibold">{label} · tối đa 20 điểm<input type="number" min={0} max={20} step={0.5} required value={value} onChange={(event) => onChange(Number(event.target.value))} className="rounded border p-2 font-normal" /></label>; }
