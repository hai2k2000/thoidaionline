      ...
      <main className="min-w-0 flex-1 space-y-2.5">
        ...
        <div className="grid items-start gap-2.5 lg:grid-cols-[minmax(0,1fr)_300px]">
          ...
          <aside aria-label="Thông tin nhanh" className="order-first space-y-2.5 lg:order-none lg:sticky lg:top-3">
            ...
            <Section title="Trao đổi"><div className="rounded-xl border bg-slate-50 p-2.5">{capabilities.comment ? <form onSubmit={submitComment} className="mb-3 grid gap-2"><textarea value={comment} onChange={(event) => setComment(event.target.value)} required maxLength={5000} placeholder="Nhập nội dung trao đổi..." className="min-h-16 resize-y rounded-2xl border bg-white p-3 text-sm" /><div className="flex items-center gap-2"><label title="Đính kèm tệp" aria-label="Đính kèm tệp" className="flex h-9 w-9 cursor-pointer items-center justify-center rounded-full border bg-white hover:bg-orange-50"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-5 w-5"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><path d="M14 2v6h6M12 12v6m-3-3h6"/></svg><input name="file" type="file" accept=".pdf,.png,.jpg,.jpeg,.docx,.xlsx" className="sr-only" /></label><button disabled={busy} className="rounded-full bg-orange-600 px-4 py-2 text-sm font-semibold text-white">Gửi</button></div></form> : null}{task.comments.length ? <ul aria-label="Danh sách trao đổi" className="max-h-[230px] space-y-2.5 overflow-y-auto overscroll-contain pr-1">{task.comments.map((row) => <li key={row.id} className="rounded-2xl rounded-tl-md bg-white px-3 py-2 shadow-sm"><div className="flex flex-col gap-0.5"><strong className="text-xs text-slate-800">{row.staff_users?.full_name ?? "Người dùng"}</strong><time className="text-[10px] text-slate-400">{dateText(row.created_at)}</time></div><p className="mt-1 whitespace-pre-wrap break-words text-sm text-slate-700">{row.content}</p></li>)}</ul> : <p className="rounded-2xl bg-white p-3 text-sm text-slate-500">Chưa có trao đổi.</p>}</div></Section>
