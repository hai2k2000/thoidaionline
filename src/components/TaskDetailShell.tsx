          ...
          <div className="mt-2 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            ...
            <div className="flex max-w-full shrink-0 flex-nowrap items-center gap-1 overflow-x-auto pb-1 sm:justify-end">
              ...
              {hasTaskActions ? <>
                ...
                {!personal && capabilities.review && task.status === "pending_review" ? <><button disabled={busy} onClick={() => requirements.length ? document.getElementById("task-scoring-form")?.scrollIntoView({ behavior: "smooth" }) : jsonPost(`/api/tasks/${task.id}/review-completion`, { decision: "approve" })} className="shrink-0 whitespace-nowrap rounded bg-emerald-700 px-2 py-1.5 text-xs font-semibold text-white">{requirements.length ? "Chấm điểm" : "Duyệt"}</button><button disabled={busy} onClick={returnWithDeadline} className="shrink-0 whitespace-nowrap rounded bg-amber-600 px-2 py-1.5 text-xs font-semibold text-white">Trả lại</button></> : null}
        ...
        <div className="grid items-start gap-2.5 lg:grid-cols-[minmax(0,1fr)_300px]">
          <div className="min-w-0 space-y-2.5">
            <section aria-label="Tổng quan" className="space-y-2.5">
              ...
              <Section title="Trao đổi"><div className="rounded-xl border bg-slate-50 p-3">{capabilities.comment ? <form onSubmit={submitComment} className="mb-4 grid gap-2"><textarea value={comment} onChange={(event) => setComment(event.target.value)} required maxLength={5000} placeholder="Nhập nội dung trao đổi..." className="min-h-20 rounded-2xl border bg-white p-3" /><div className="flex items-center gap-2"><label title="Đính kèm tệp" aria-label="Đính kèm tệp" className="flex h-10 w-10 cursor-pointer items-center justify-center rounded-full border bg-white text-xl hover:bg-orange-50"><span aria-hidden="true">📎</span><input name="file" type="file" accept=".pdf,.png,.jpg,.jpeg,.docx,.xlsx" className="sr-only" /></label><button disabled={busy} className="rounded-full bg-orange-600 px-5 py-2 font-semibold text-white">Gửi</button></div></form> : null}{task.comments.length ? <ul aria-label="Danh sách trao đổi" className="max-h-96 space-y-3 overflow-y-auto pr-1">{task.comments.map((row) => <li key={row.id} className="max-w-[90%] rounded-2xl rounded-tl-md bg-white px-3 py-2 shadow-sm"><div className="flex items-baseline justify-between gap-3"><strong className="text-sm text-slate-800">{row.staff_users?.full_name ?? "Người dùng"}</strong><time className="text-xs text-slate-400">{dateText(row.created_at)}</time></div><p className="mt-1 whitespace-pre-wrap break-words text-sm text-slate-700">{row.content}</p></li>)}</ul> : <p className="rounded-2xl bg-white p-3 text-sm text-slate-500">Chưa có trao đổi.</p>}</div></Section>
              {capabilities.review && requirements.length ? <Section title="Chấm điểm hoàn thành"><p className="mb-3 text-sm text-slate-600">Cơ cấu điểm: đáp ứng yêu cầu 60 điểm · thái độ và phối hợp 20 điểm · chủ động và trách nhiệm 20 điểm.</p>{task.status === "pending_review" ? <form id="task-scoring-form" onSubmit={submitScoreAndApprove} className="grid gap-4"><fieldset className="grid gap-2"><legend className="font-semibold">1. Mức độ đáp ứng yêu cầu · tối đa 60 điểm</legend>{requirements.map((item, index) => <label key={index} className="flex items-start gap-2 rounded-lg border p-3"><input type="checkbox" checked={requirementResults[index]} onChange={(event) => { const next = requirementResults.map((value, itemIndex) => itemIndex === index ? event.target.checked : value); setRequirementResults(next); setRequirementScore(Number((next.filter(Boolean).length * 60 / requirements.length).toFixed(2))); }} className="mt-1" /><span>{item}</span></label>)}</fieldset><label className="grid gap-1 text-sm font-semibold">Điểm đáp ứng yêu cầu · tự động tính, có thể điều chỉnh<input type="number" min={0} max={60} step={0.01} required value={requirementScore} onChange={(event) => setRequirementScore(Number(event.target.value))} className="rounded border p-2 font-normal" /></label><ScoreField label="2. Thái độ chuyên nghiệp và năng lực phối hợp" value={collaborationScore} onChange={setCollaborationScore} /><ScoreField label="3. Tinh thần chủ động, tiên phong và trách nhiệm" value={initiativeScore} onChange={setInitiativeScore} /><label className="grid gap-1 text-sm font-semibold">Nhận xét<textarea value={scoreNote} onChange={(event) => setScoreNote(event.target.value)} maxLength={2000} className="min-h-20 rounded border p-2 font-normal" /></label><p className="font-bold text-emerald-800">Tổng điểm dự kiến: {(requirementScore + collaborationScore + initiativeScore).toFixed(2)}/100</p><button disabled={busy} className="rounded bg-emerald-700 px-4 py-2 font-semibold text-white sm:justify-self-start">Lưu điểm và duyệt hoàn thành</button></form> : <p className="rounded-lg bg-slate-50 p-3 text-sm text-slate-600">Chấm điểm sẽ mở khi người thực hiện gửi công việc để duyệt.</p>}</Section> : null}
