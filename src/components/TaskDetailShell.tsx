...
type Capabilities = {
  ...
  attachment: boolean; personalComplete: boolean; personalCancel: boolean;
};
      ...
      <main className="min-w-0 flex-1 space-y-4">
        ...
        <Section title="Báo cáo tiến độ">
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {!personal && capabilities.report && !["pending_review", "done", "cancelled"].includes(task.status) ? <button disabled={busy} onClick={() => jsonPost(`/api/tasks/${task.id}/submit-completion`)} className="rounded bg-emerald-700 px-3 py-2 text-white">Gửi hoàn thành</button> : null}
            ...
            {personal && capabilities.personalComplete ? <button disabled={busy} onClick={() => jsonPost(`/api/tasks/${task.id}/complete`)} className="rounded bg-emerald-700 px-3 py-2 text-white">Hoàn thành</button> : null}
            {((personal && capabilities.personalCancel) || (!personal && capabilities.update && !["done", "cancelled"].includes(task.status))) ? <button disabled={busy} onClick={() => reasonAction(`/api/tasks/${task.id}/${personal ? "cancel" : "cancel-assigned"}`, "Hủy")} className="rounded bg-red-700 px-3 py-2 text-white">Hủy</button> : null}
            {!personal && capabilities.update && !["done", "cancelled"].includes(task.status) ? <button disabled={busy} onClick={async () => { const dueDate = window.prompt("Hạn mới (YYYY-MM-DD):", task.due_date ?? ""); if (dueDate) await reasonAction(`/api/tasks/${task.id}/deadline-assigned`, "Đổi hạn", { dueDate }); }} className="rounded border px-3 py-2">Đổi hạn</button> : null}
          </div>
        </Section>
        ...
        <Section title="Đánh giá cũ"><p className="mb-2 text-sm text-slate-500">Dữ liệu cũ chỉ đọc.</p><Timeline empty="Chưa có đánh giá cũ.">{task.legacy_evaluations.map((row) => <li key={row.id}><b>{row.is_final ? "Đánh giá cuối kỳ" : "Đánh giá giữa kỳ"} · Ngày đánh giá: {row.checkpoint_date} · {row.rating}/10</b><p>Ý kiến đánh giá: {row.opinion || "—"}</p></li>)}</Timeline></Section>
