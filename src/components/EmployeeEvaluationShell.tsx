...
const labels: Record<string, string> = { self_draft: "Chờ Trưởng phòng", awaiting_manager: "Chờ Trưởng phòng", awaiting_tbt: "Chờ Tổng Biên tập", published: "Đã công bố" };

function ScoreForm({ item, action }: { item: EvaluationItem; action: "manager" | "tbt" }) {
  const router = useRouter(); const { notify } = useActionFeedback(); const [busy, setBusy] = useState(false); const [error, setError] = useState(false);
  const submit = async (formData: FormData) => {
    ...
    try { const response = await fetch(`/api/evaluations/${item.id}`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action, scores }) });
    if (!response.ok) throw new Error(await responseErrorMessage(response, "Không thể lưu đánh giá."));
    notify("success", action === "tbt" ? "Đã công bố kết quả đánh giá." : "Đã gửi đánh giá."); router.refresh();
    } catch (error) { setError(true); notify("error", errorMessage(error, "Không thể lưu đánh giá.")); } finally { setBusy(false); }
  };
  return <form action={submit} className="mt-4 rounded-lg border bg-slate-50 p-3">
    <p className="font-semibold">{action === "manager" ? "Trưởng phòng — bước 1" : "Tổng Biên tập — bước 2 và công bố"}</p>
...
export default function EmployeeEvaluationShell({ data }: { data: EvaluationPageData }) {
  return <section className="mt-4 space-y-4">
    ...
    {data.items.map((item) => <article key={item.id} className="rounded-xl border bg-white p-4 shadow-sm">
      ...
      {item.canManager ? <ScoreForm item={item} action="manager" /> : null}{item.canTbt ? <ScoreForm item={item} action="tbt" /> : null}
    </article>)}
