...
export default function PublicEvaluationSummary({ userLabel }: { userLabel: string }) {
  ...
  useEffect(() => {
    fetch(`/api/public-evaluation-summary?period=${period}`)
    ...
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-4 lg:flex-row lg:gap-6">
      ...
      <main className="min-w-0 flex-1">
        <section ref={panelRef} className="rounded-2xl border bg-white p-4 shadow-sm sm:p-5 fullscreen:overflow-auto fullscreen:p-6">
          ...
          <div className="mt-4 flex flex-wrap items-center gap-2"><label className="text-sm font-semibold text-slate-600">Kỳ đánh giá</label><select value={period} onChange={(e) => { setPeriod(e.target.value); setOpen(null); }} className="rounded-xl border bg-white px-3 py-2 text-sm"><option value="all">Tất cả thời gian</option><option value="month">Tháng hiện tại</option><option value="quarter">Quý hiện tại</option><option value="year">Năm hiện tại</option></select></div>
