...
const successMessages: Record<Action, string> = {
  ...
  withdraw: "Đã chuyển bài sang trạng thái Đã rút.",
};
...
export default function JournalismPublicationControls({ journalism, taskId }: { journalism: JournalismTaskDetailDto; taskId: string }) {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (action === "schedule") {
      ...
      if (!result.ok) return setError(reason.trim() ? "Lý do rút bài không được vượt quá 2.000 ký tự." : "Vui lòng nhập lý do rút bài.");
