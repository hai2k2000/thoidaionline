...
const answers: Answer[] = [
  {
    title: "Tạo và giao công việc",
    keywords: ["tạo", "giao", "nhiệm vụ", "công việc"],
    text: `Cách tạo và giao công việc:
1. Mở menu Giao việc.
...
Sau khi tạo, công việc xuất hiện trong Quản lý công việc. Nếu không thấy người cần chọn, kiểm tra tài khoản còn hoạt động và đúng phòng ban/chức vụ.`,
  },
  {
    ...
    keywords: ["tiến độ", "báo cáo", "vướng mắc", "bị chặn", "đang chờ"],
    text: `Cách báo cáo tiến độ:
...
Báo cáo tiến độ chưa phải là hoàn thành. Khi đã xong, dùng nút Hoàn thành ở góc trên bên phải.`,
  ...
  {
    title: "Hoàn thành và duyệt",
    keywords: ["hoàn thành", "xong", "duyệt", "trả lại"],
    text: `Quy trình hoàn thành:
...
Nếu không thấy nút Hoàn thành, kiểm tra bạn có phải người thực hiện hoặc công việc đã hoàn thành/đã hủy hay chưa.`,
  ...
  {
    title: "Quản lý và tìm công việc",
    keywords: ["danh sách", "lọc", "tìm", "quản lý", "được giao", "theo dõi"],
    text: `Trong Quản lý công việc:
...
Nếu danh sách trống, hãy bấm Đặt lại rồi kiểm tra lại phạm vi đang chọn.`,
  },
  {
    ...
    keywords: ["cá nhân", "kế hoạch", "tự tạo", "đổi ngày", "hủy nhiệm vụ"],
    text: `Cách tạo nhiệm vụ cá nhân:
...
Nếu cần thêm, xóa hoặc đổi người trong lịch, vui lòng liên hệ quản trị viên.`,
  },
  {
    ...
    keywords: ["đánh giá", "chấm điểm", "nhận xét", "nhân viên", "lãnh đạo"],
...
Nếu không thấy menu, admin cần kiểm tra role và quyền đánh giá của tài khoản.`,
  ...
  {
    title: "Đổi hoặc quên mật khẩu",
    keywords: ["mật khẩu", "password", "đăng nhập", "quên", "sai mật khẩu"],
    text: `Đổi mật khẩu của bạn:
1. Mở menu Tài khoản.
...
Nếu quên: bấm Quên mật khẩu? ở màn hình đăng nhập. Nếu mật khẩu đúng nhưng vẫn không vào được, hãy liên hệ quản trị viên để kiểm tra tài khoản.`,
  },
  {
    ...
    keywords: ["bình luận", "comment", "theo dõi", "trao đổi"],
    text: `Cách trao đổi:
...
Người thực hiện và người theo dõi được bình luận. Khi giao việc, trưởng phòng của người thực hiện được theo dõi mặc định và có thể chọn thêm người.`,
  ...
  {
    title: "Tệp đính kèm",
    keywords: ["file", "tệp", "đính kèm", "tải lên", "upload"],
    text: `Cách tải tệp:
...
const find = (q: string) => {
  ...
  const s = answers
    .map((answer) => ({
      answer,
      score: answer.keywords.filter((k) => n.includes(norm(k))).length,
    }))
    .sort((a, b) => b.score - a.score);
  return s[0]?.score ? s[0].answer : null;
};
export default function HelpBot() {
  ...
  >([
    {
      from: "bot",
      text: "Xin chào! Hãy mô tả việc muốn làm. Tôi sẽ hướng dẫn từng bước và nêu cách xử lý lỗi thường gặp.",
    },
  ]);
  ...
  return (
    <div className="fixed bottom-4 right-4 z-[70]">
      {open ? (
        <section
          aria-label="Trợ lý hướng dẫn sử dụng"
          className="mb-2 flex h-[min(620px,calc(100vh-6rem))] w-[min(430px,calc(100vw-2rem))] flex-col overflow-hidden rounded-2xl border bg-white shadow-2xl"
        >
          <header className="flex items-center justify-between bg-orange-600 px-4 py-3 text-white">
            <div>
              <p className="font-bold">Trợ lý Thời Đại Work</p>
          ...
          <div className="min-h-0 flex-1 space-y-3 overflow-y-auto p-3 text-sm leading-6">
            {messages.map((m, i) => (
              <div
                key={i}
                className={`max-w-[94%] whitespace-pre-wrap rounded-xl px-3 py-2 ${m.from === "user" ? "ml-auto bg-slate-100" : "bg-orange-50"}`}
              >
                {m.text}
              </div>
            ))}
