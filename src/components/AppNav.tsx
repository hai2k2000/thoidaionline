...
function NavIcon({ id }: { id: keyof typeof labels }) {
...
function NavContent({
  ...
  return (
    <div className="flex h-full flex-col p-3.5">
      <div className="rounded-2xl bg-gradient-to-br from-orange-50 via-white to-amber-50 px-3 py-3.5 text-center ring-1 ring-orange-100">
      ...
      <p className="mb-1.5 mt-5 px-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-400">Công việc</p>
      <nav aria-label="Menu chính" className="space-y-1">
        {navigation.primary.map((item) => (
          ...
          >
            <NavIcon id={item.id} /><span className="leading-snug">{labels[item.id]}</span>
          </Link>
        ))}
      </nav>
      ...
      <div className="mt-auto border-t border-slate-200 pt-3">
        <div className="mb-2.5 flex items-center gap-2.5 rounded-xl bg-gradient-to-r from-slate-50 to-orange-50 px-3 py-2.5 ring-1 ring-slate-200">
        ...
        {navigation.account.map((item) => (
          <Link
            ...
            className={linkClass(isNavigationActive(currentPath, item.href))}
          >
            <NavIcon id={item.id} /><span>{labels[item.id]}</span>
          </Link>
        ))}
        <button
          type="button"
          ...
          className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl border border-orange-200 bg-white px-3 py-2.5 text-sm font-semibold text-orange-700 transition hover:bg-orange-50 hover:text-orange-900"
        >
