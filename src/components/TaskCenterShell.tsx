    ...
    : status === "pending_review" ? (approvalRequired ? "Chờ duyệt hoàn thành" : "Chờ chấm điểm")
      ...
      : status === "rejected" ? "Trả lại"
        : status === "cancelled" ? "Đã hủy"
          : "Chưa hoàn thành";
const assignmentSourceLabel = (source?: string) => source === "self_registered" ? "Tự đăng ký" : source === "leadership_assigned" ? "Lãnh đạo giao" : "Không xác định (dữ liệu cũ)";
...
function FilterFields({ query, departments, journalismWorkKinds, journalismTopics, journalismSeries, canAccessJournalism, basePath = "/tasks" }: Pick<Props, "query" | "departments" | "journalismWorkKinds" | "journalismTopics" | "journalismSeries" | "canAccessJournalism" | "basePath">) {
  ...
  return (
    <>
      ...
      <select name="assignmentSource" defaultValue={query.assignmentSource ?? ""} className="w-full min-w-0 rounded-lg border px-3 py-2.5">
        <option value="">Hình thức · Tất cả</option><option value="leadership_assigned">Lãnh đạo giao</option><option value="self_registered">Tự đăng ký</option>
      </select>
...
function ApprovalQueue({ queue, data, onRefresh }: { queue: "assignment" | "completion"; data: TaskListResult; onRefresh: () => void }) {
  ...
  return <div className="space-y-3">
    ...
    {!rows.length ? <p className="rounded-lg bg-slate-50 p-6 text-center text-slate-600">Không có công việc chờ duyệt.</p> : <div className="grid gap-3">{rows.map((task) => <article key={task.id} className="rounded-xl border border-orange-100 bg-orange-50/30 p-4"><div className="flex flex-wrap items-start justify-between gap-3"><div><Link href={`/tasks/${task.id}`} className="font-bold text-orange-900 hover:underline">{task.title}</Link><p className="mt-1 text-sm text-slate-600">{semanticAssigner(task).label}: {semanticAssigner(task).value} → {task.owner_id === task.created_by ? "Tự thực hiện" : assigneeDisplay(task)} · {task.departments?.name ?? "—"}</p></div><span className="rounded-full bg-amber-100 px-2.5 py-1 text-xs font-bold text-amber-800">{taskStatusLabel(task.status, task.approval_required)}</span></div><dl className="mt-3 grid gap-2 text-sm sm:grid-cols-4"><SummaryItem label="Ngày tạo" value={task.created_at.slice(0, 10).split("-").reverse().join("/")} /><SummaryItem label="Hạn hoàn thành" value={dueText(task.due_date, task.due_time)} /><SummaryItem label="Hình thức" value={assignmentSourceLabel(task.assignment_source)} /><SummaryItem label="Loại" value={task.task_type === "personal" ? "Tự tạo" : "Được giao"} /><SummaryItem label="Người thực hiện" value={assigneeDisplay(task).toString()} /></dl>{task.recent_workflow_event ? <div className="mt-3 rounded-lg border border-orange-100 bg-white p-3 text-sm"><p className="font-semibold text-slate-700">Lịch sử xử lý gần đây: {task.recent_workflow_event.label}</p><p className="mt-1 text-slate-600">{task.recent_workflow_event.actor_name ?? "Người dùng"} · {task.recent_workflow_event.created_at.slice(0, 16).replace("T", " ")}</p>{task.recent_workflow_event.reason ? <p className="mt-1 text-slate-700">Lý do: {task.recent_workflow_event.reason}</p> : null}</div> : null}<div className="mt-3 flex flex-wrap gap-2"><button disabled={busy === task.id} onClick={() => void act(task.id, "approve")} className="rounded-lg bg-emerald-700 px-3 py-2 text-sm font-semibold text-white">{queue === "assignment" ? "Duyệt giao việc" : "Duyệt hoàn thành"}</button><button disabled={busy === task.id} onClick={() => void act(task.id, queue === "assignment" ? "reject" : "return")} className="rounded-lg border border-amber-300 bg-white px-3 py-2 text-sm font-semibold text-amber-800">{queue === "assignment" ? "Từ chối giao việc" : "Yêu cầu làm lại"}</button></div></article>)}</div>}
  </div>;
}
                ...
                {tasks.items.length ? (
                  <>
                    ...
                    <table className="data-table w-full min-w-[1120px] border-collapse text-sm">
                      <thead><tr className="border-b text-left text-slate-600"><th scope="col" className="w-12 p-2">STT</th><th scope="col" className="p-2">Tên công việc</th><th scope="col" className="p-2">Hình thức</th><th scope="col" className="p-2">Người giao</th><th scope="col" className="p-2">Người nhận việc</th><th scope="col" className="p-2">Người theo dõi</th><th scope="col" className="p-2">Hạn hoàn thành</th><th scope="col" className="p-2">Điểm</th><th scope="col" className="p-2">Trạng thái</th></tr></thead>
