                    ...
                    <table className="w-full min-w-[980px] border-collapse text-sm">
                      ...
                      <tbody>{tasks.items.map((task) => {
                        ...
                        return <tr key={task.id} role="link" tabIndex={0} onClick={() => router.push(`/tasks/${task.id}`)} onKeyDown={(event) => { if (event.key === "Enter") router.push(`/tasks/${task.id}`); }} className="cursor-pointer border-b hover:bg-slate-50 focus:bg-slate-50">
                          <td className="p-2">{participantNames(task, "assignee")}</td><td className="p-2">{task.created_by_user?.full_name ?? "—"}</td><td className="p-2">{participantNames(task, "watcher")}</td><td className="p-2">{dueText(task.due_date, task.due_time)}</td><td className="p-2 font-semibold">{task.completion_score?.total_score ?? "—"}</td><td className="p-2 font-semibold">{task.title}{task.legacy_read_only ? <span className="ml-2 rounded bg-amber-100 px-2 py-0.5 text-xs">Dữ liệu cũ · chỉ đọc</span> : null}</td><td className="p-2"><span className={`inline-flex rounded-full px-2.5 py-1 text-xs font-bold ${statusClass(task.status)}`}>{taskStatusLabel(task.status)}</span></td><td className="p-2">{task.departments?.name ?? "—"}</td><td className="p-2">{task.task_category === "duty" ? "Trực sản xuất" : task.compatibility_task_type === "personal" ? "Nhiệm vụ cá nhân" : task.compatibility_task_type === "assigned" ? "Công việc được giao" : "Dữ liệu cũ"}</td><td className="p-2">{task.start_date ?? "—"}</td>
