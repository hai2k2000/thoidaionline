              ...
              <section className="mt-3 overflow-hidden rounded-xl border bg-white p-3 shadow-sm">
                ...
                {tasks.items.length ? (
                  <>
                    ...
                    {!taskMode ? <details className="mt-4 rounded-xl border border-orange-200 bg-orange-50/40"><summary className="cursor-pointer px-4 py-3 font-bold text-orange-900">Lịch công tác <span className="ml-2 text-xs font-normal text-slate-600">TBT · Phó TBT · Trưởng phòng · Phóng viên</span></summary><div className="border-t bg-white p-3"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{tasks.items.filter((task) => task.due_date).sort((a, b) => (a.due_date ?? "").localeCompare(b.due_date ?? "")).map((task) => <Link key={task.id} href={`/tasks/${task.id}`} className="rounded-lg border p-3 hover:border-orange-300 hover:bg-orange-50"><p className="text-xs font-semibold text-orange-700">{dueText(task.due_date, task.due_time)}</p><p className="mt-1 font-semibold">{task.title}</p><p className="mt-1 text-sm text-slate-600">{participantNames(task, "assignee")}</p><p className="text-xs text-slate-500">Giao: {task.created_by_user?.full_name ?? "—"}</p></Link>)}{!tasks.items.some((task) => task.due_date) ? <p className="text-sm text-slate-500">Chưa có lịch công tác trong phạm vi này.</p> : null}</div></div></details> : null}
                  </>
                ) : null}
