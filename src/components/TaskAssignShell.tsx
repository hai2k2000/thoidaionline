...
export default function TaskAssignShell({ departments, people, userLabel, canManageEventAssignment, journalismMode, journalismWorkKinds, journalismWorkKindsLoaded }: {
  ...
  const managerLabel = selectedDepartment?.managerId
    ...
    : null;
  const watcherPeople = journalismMode ? scopedPeople : people;
    ...
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-3 lg:flex-row lg:gap-4">
      ...
      <main className="min-w-0 flex-1">
        ...
        <form onSubmit={submit} className="mt-3 grid items-start gap-x-4 gap-y-3 rounded-xl border bg-white p-4 shadow-sm lg:grid-cols-2">
          ...
          <Field label="Người theo dõi bổ sung"><CheckGroup name="watcherIds" people={watcherPeople.filter((person) => person.id !== assigneeId && !collaboratorIds.includes(person.id))} selected={watcherIds} onChange={setWatcherIds} empty="Không còn người phù hợp." /><span className="font-normal text-slate-500">Trưởng phòng chính được thêm tự động; lựa chọn trùng sẽ được gộp.</span></Field>
