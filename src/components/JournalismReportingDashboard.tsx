...
const statusLabels: Record<string, string> = {
  new: "Mới", in_progress: "Đang làm", blocked: "Có vướng mắc", waiting: "Chờ nhận việc",
  pending_review: "Chờ duyệt", rejected: "Trả lại", done: "Hoàn thành", cancelled: "Đã hủy",
};
const publicationLabels: Record<string, string> = {
  not_published: "Chưa ghi nhận xuất bản", scheduled: "Đã lên lịch", published: "Đã xuất bản", withdrawn: "Đã gỡ",
};
  ...
  return <div className="min-h-screen bg-slate-50 px-3 py-4 text-slate-900 sm:px-4 lg:px-6">
    <div className="mx-auto flex w-full max-w-[1500px] flex-col gap-3 lg:flex-row lg:gap-4">
      ...
      <main className="min-w-0 flex-1 space-y-3">
        <header className="rounded-2xl border bg-white p-4 shadow-sm sm:p-5"><p className="text-xs font-bold uppercase tracking-wide text-orange-600">Journalism · Báo cáo nội bộ</p><h1 className="mt-1 text-2xl font-bold sm:text-3xl">Báo cáo nghiệp vụ báo chí</h1><p className="mt-2 max-w-3xl text-sm text-slate-600">Tổng hợp từ Journalism Task, báo cáo xuất bản thủ công và lịch sử xác minh trong Thời Đại Work. Đây là dữ liệu nội bộ, không phải xác nhận từ hệ thống bên ngoài.</p></header>
        <form method="get" action="/journalism/reports" className="grid gap-3 rounded-xl border bg-white p-3 shadow-sm sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5">
          <label className="grid gap-1 text-xs font-semibold text-slate-600">Task tạo từ<input name="from" type="date" defaultValue={query.fromDate} className="rounded-lg border px-3 py-2 text-sm font-normal" /></label>
          <label className="grid gap-1 text-xs font-semibold text-slate-600">Task tạo đến<input name="to" type="date" defaultValue={query.toDate} className="rounded-lg border px-3 py-2 text-sm font-normal" /></label>
          ...
          <label className="grid gap-1 text-xs font-semibold text-slate-600">Topic<select name="topic" defaultValue={query.topicId ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm font-normal"><option value="">Tất cả</option>{options.topics.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label>
          <label className="grid gap-1 text-xs font-semibold text-slate-600">Series<select name="series" defaultValue={query.seriesId ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm font-normal"><option value="">Tất cả</option>{options.series.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select></label>
          <label className="grid gap-1 text-xs font-semibold text-slate-600">Trạng thái Task<select name="status" defaultValue={query.status ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm font-normal"><option value="">Tất cả</option>{Object.entries(statusLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label className="grid gap-1 text-xs font-semibold text-slate-600">Trạng thái nghiệp vụ xuất bản<select name="publicationStatus" defaultValue={query.publicationStatus ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm font-normal"><option value="">Tất cả</option>{Object.entries(publicationLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
        ...
        <p className="text-xs text-slate-500">Khoảng thời gian chính dựa trên ngày tạo Journalism Task: {date(query.fromDate)} – {date(query.toDate)}.</p>
        ...
        {!error && data ? <>
          ...
          <section aria-label="Chỉ số tổng quan" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7"><Kpi label="Tổng Journalism Task" value={data.totalTasks} /><Kpi label="Đã ghi nhận xuất bản" value={data.reportedPublications} hint="Có J6D manual publication report" /><Kpi label="Chưa ghi nhận xuất bản" value={data.unreportedTasks} /><Kpi label="Đã xác minh" value={data.verificationVerified} /><Kpi label="Chưa xác minh" value={data.verificationUnverified} /><Kpi label="Bị từ chối" value={data.verificationRejected} /><Kpi label="Cần xác minh lại" value={data.verificationStale} /><Kpi label="Quá hạn" value={data.overdue} /></section>
          <div className="grid items-start gap-3 xl:grid-cols-2"><ReportTable title="Theo người phụ trách" headers={["Người phụ trách", "Task", "Báo cáo", "Đã xác minh", "Từ chối", "Stale", "Quá hạn"]} rows={data.byAssignee.map((row) => [row.name, row.tasks, row.reports, row.verified, row.rejected, row.stale, row.overdue])} empty="Chưa có dữ liệu người phụ trách trong phạm vi này." /><ReportTable title="Theo phòng ban" headers={["Phòng ban", "Task", "Báo cáo", "Đã xác minh", "Từ chối", "Stale", "Quá hạn"]} rows={data.byDepartment.map((row) => [row.name, row.tasks, row.reports, row.verified, row.rejected, row.stale, row.overdue])} empty="Chưa có dữ liệu phòng ban trong phạm vi này." /></div>
          <div className="grid items-start gap-3 xl:grid-cols-2"><ReportTable title="Theo Topic" headers={["Topic", "Task", "Báo cáo", "Đã xác minh"]} rows={data.byTopic.map((row) => [row.name, row.tasks, row.reports, row.verified])} empty="Chưa có Topic trong phạm vi này." /><ReportTable title="Theo Series" headers={["Series", "Task", "Báo cáo", "Đã xác minh"]} rows={data.bySeries.map((row) => [row.name, row.tasks, row.reports, row.verified])} empty="Chưa có Series trong phạm vi này." /></div>
          ...
          <section className="rounded-xl border bg-white p-3 shadow-sm"><h2 className="text-lg font-bold">Cần chú ý</h2><p className="mt-1 text-xs text-slate-500">Danh sách chỉ đọc; thao tác xử lý vẫn thực hiện ở Task detail.</p><div className="mt-3 grid gap-2 sm:grid-cols-2">{data.attention.map((row) => <Link key={`${row.kind}-${row.taskId}`} href={`/tasks/${row.taskId}`} className="rounded-lg border p-3 transition hover:border-orange-300 hover:bg-orange-50"><span className="text-xs font-semibold text-amber-700">{row.label}</span><span className="mt-1 block font-semibold">{row.taskTitle}</span></Link>)}</div>{!data.attention.length ? <p className="p-5 text-center text-sm text-slate-500">Không có mục cần chú ý.</p> : null}</section>
        </> : null}
      </main>
    </div>
  </div>;
