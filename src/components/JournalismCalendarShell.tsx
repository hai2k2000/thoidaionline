...
import { classifyCalendarTask, type CalendarPublicationStatus, type JournalismCalendarQuery } from "@/lib/journalismCalendar";
import type { JournalismCalendarResult, JournalismCalendarTask } from "@/lib/journalismCalendarRepository";
import { journalismCalendarStatusLabels, journalismLabels, journalismPublicationStatusLabel } from "@/lib/journalismUi.mjs";

const statusLabels = journalismCalendarStatusLabels;
const statusClass = {
...
export default function JournalismCalendarShell({ userLabel, query, data }: Props) {
  ...
  return <div className="min-h-screen bg-slate-50 px-3 py-4 text-slate-900 sm:px-4 lg:px-6"><div className="mx-auto flex w-full max-w-[1500px] flex-col gap-3 lg:flex-row lg:gap-4"><AppNav currentPath="/journalism/calendar" userLabel={userLabel} /><main className="min-w-0 flex-1 space-y-3">
    <header className="rounded-2xl border bg-white p-4 shadow-sm"><p className="text-xs font-bold uppercase tracking-wide text-orange-600">Lịch nghiệp vụ báo chí · J7 Lite</p><h1 className="mt-1 text-2xl font-bold sm:text-3xl">Lịch nghiệp vụ phòng Nội dung</h1><p className="mt-2 max-w-3xl text-sm text-slate-600">Lập kế hoạch thủ công theo ngày dự kiến xuất bản của công việc nghiệp vụ báo chí. Dữ liệu chỉ trong phạm vi phòng Nội dung.</p></header>
    <div className="flex flex-wrap gap-2 rounded-xl border bg-white p-3 shadow-sm">{(["day", "week", "month"] as const).map((view) => <Link key={view} href={href({ view })} className={`rounded-lg px-3 py-2 text-sm font-semibold ${query.view === view ? "bg-orange-500 text-white" : "border bg-white"}`}>{view === "day" ? "Ngày" : view === "week" ? "Tuần" : "Tháng"}</Link>)}<form method="get" className="flex flex-wrap gap-2"><input type="hidden" name="view" value={query.view} /><input type="date" name="date" defaultValue={query.anchorDate} className="rounded-lg border px-3 py-2 text-sm" /><select name="reporter" defaultValue={query.reporterId ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm"><option value="">Tất cả phóng viên</option>{options.reporters.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select name="status" defaultValue={query.publicationStatus ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm"><option value="">Mọi trạng thái xuất bản</option><option value="not_published">{journalismPublicationStatusLabel("not_published")}</option><option value="scheduled">{journalismPublicationStatusLabel("scheduled")}</option><option value="published">{journalismPublicationStatusLabel("published")}</option><option value="withdrawn">{journalismPublicationStatusLabel("withdrawn")}</option></select><select name="topic" defaultValue={query.topicId ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm"><option value="">Mọi Chủ đề</option>{options.topics.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><select name="series" defaultValue={query.seriesId ?? ""} className="rounded-lg border bg-white px-3 py-2 text-sm"><option value="">Mọi Loạt bài</option>{options.series.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><button className="rounded-lg bg-slate-900 px-4 py-2 text-sm font-semibold text-white">Lọc</button></form></div>
    ...
    <section className="rounded-xl border bg-white p-3 shadow-sm"><div className="mb-3 flex flex-wrap items-center justify-between gap-2"><div><h2 className="text-lg font-bold">{data.from} → {data.to}</h2><p className="text-xs text-slate-500">{tasks.length} công việc nghiệp vụ báo chí trong phạm vi được phép xem.</p></div><div className="flex flex-wrap gap-1 text-xs">{Object.entries(statusLabels).map(([status, label]) => <span key={status} className={`rounded-full border px-2 py-1 ${statusClass[status as keyof typeof statusClass]}`}>{label}</span>)}</div></div><div className="space-y-3">{groups.map(([date, items]) => <div key={date} className="rounded-lg border p-3"><h3 className="font-semibold">{date === "unplanned" ? "Chưa lên kế hoạch" : date}</h3><div className="mt-2 grid gap-2">{items.map((task) => { const status = classifyCalendarTask({ plannedPublicationAt: task.plannedPublicationAt, publicationStatus: task.publicationStatus as CalendarPublicationStatus }, today); return <article key={task.id} className={`rounded-lg border p-3 ${statusClass[status]}`}><div className="flex flex-wrap items-start justify-between gap-2"><div><Link href={`/tasks/${task.id}`} className="font-semibold underline">{task.title}</Link><p className="mt-1 text-xs">{task.assigneeName ?? "Chưa gán phóng viên"}{task.topic ? ` · ${journalismLabels.topic}: ${task.topic.name}` : ""}{task.series ? ` · ${journalismLabels.series}: ${task.series.name}` : ""}</p><p className="mt-1 text-xs">{statusLabels[status]} · {dateLabel(task.plannedPublicationAt)}</p></div><form onSubmit={(event) => { event.preventDefault(); const input = event.currentTarget.elements.namedItem("planned") as HTMLInputElement; void saveDate(task.id, input.value); }} className="flex items-center gap-2"><input name="planned" type="datetime-local" defaultValue={task.plannedPublicationAt ? task.plannedPublicationAt.slice(0, 16) : ""} className="rounded border bg-white px-2 py-1 text-xs" aria-label={`Ngày dự kiến xuất bản cho ${task.title}`} /><button className="rounded bg-white px-2 py-1 text-xs font-semibold ring-1 ring-slate-300">Lưu ngày</button></form></div></article>; })}</div></div>)}{!groups.length ? <p className="p-8 text-center text-sm text-slate-500">Chưa có công việc nghiệp vụ báo chí phù hợp.</p> : null}</div></section>
  </main></div></div>;
}
