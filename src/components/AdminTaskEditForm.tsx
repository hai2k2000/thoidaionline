...
export default function AdminTaskEditForm({ task }: { task: TaskDetailDto }) {
  ...
  return <form onSubmit={submit} className="grid gap-3 rounded-xl border bg-white p-4 shadow-sm sm:grid-cols-2">
    ...
    <label className="text-sm font-semibold">Hạn hoàn thành<div className="grid grid-cols-[1fr_110px] gap-2"><input name="dueDate" type="date" required defaultValue={task.due_date ?? ""} className={control}/><input name="dueTime" type="time" defaultValue={task.due_time?.slice(0,5) ?? ""} className={control}/></div></label>
    ...
    <label className="text-sm font-semibold">Trạng thái<select name="status" defaultValue={task.status} className={control}><option value="new">Mới</option><option value="in_progress">Đang làm</option><option value="blocked">Có vướng mắc</option><option value="waiting">Chờ duyệt nhận việc</option><option value="pending_review">Chờ chấm điểm</option><option value="done">Hoàn thành</option><option value="rejected">Trả lại</option><option value="cancelled">Đã hủy</option></select></label>
