begin;
alter table public.task_recurrence_rules add column if not exists priority text not null default 'normal';
alter table public.task_recurrence_rules drop constraint if exists task_recurrence_rules_priority_check;
alter table public.task_recurrence_rules add constraint task_recurrence_rules_priority_check check (priority in ('low','normal','high','urgent'));
  ...
  if not exists(select 1 from public.staff_users where id=p_assignee_id and active=true and department_id=p_department_id)
     or not exists(
       ...
       where u.id=p_reviewer_id and u.active=true
         and ((u.department_id=p_department_id and (u.id=v_manager or (jt.active=true and lower(jt.code) in ('truong_phong','pho_truong_phong'))))
            or exists(select 1 from public.roles rr where rr.id=u.role_id and lower(rr.code) in ('tong_bien_tap','pho_tong_bien_tap')))
     )
...
revoke all on function public.api_assign_task_v2(uuid,text,text,uuid,uuid,uuid,date,time,text,uuid[],uuid[],text,date,text)
  from public,anon,authenticated;
grant execute on function public.api_assign_task_v2(uuid,text,text,uuid,uuid,uuid,date,time,text,uuid[],uuid[],text,date,text)
  to service_role;
alter function public.api_assign_task_v2(uuid,text,text,uuid,uuid,uuid,date,time,text,uuid[],uuid[],text,date,text)
  owner to postgres;

drop function if exists public.api_assign_task_v2(uuid,text,text,uuid,uuid,uuid,date,time,text,uuid[],uuid[],text,date);
