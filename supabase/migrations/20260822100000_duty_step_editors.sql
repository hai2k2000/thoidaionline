  ...
  for v_day in select value from jsonb_array_elements(p_days) loop
    ...
    for v_assignment in select value from jsonb_array_elements(v_day->'assignments') loop
      ...
      if not found
         ...
         or (v_position='Phóng viên' and (v_assignee_department is distinct from p_department_id or exists(select 1 from public.departments d where d.id=v_assignee_department and d.code='general') or lower(coalesce(v_assignee_job,'')) not in ('phong_vien','truong_phong'))) then
