...
-- Legacy pending-review rows may have no submission timestamp.  They must not
-- be approved using an unrelated historical score.
...
begin
  if new.status='done'
     ...
     and (
       new.completion_submitted_at is null
       or not exists (
         select 1 from public.task_completion_scores s
         where s.task_id=new.id
           and s.updated_at>=new.completion_submitted_at
       )
     ) then
...
begin
  ...
  if p_decision='approve' and v_before.task_category is distinct from 'duty'
     and (v_before.completion_submitted_at is null or not exists (
       select 1 from public.task_completion_scores s
       where s.task_id=p_task_id and s.updated_at>=v_before.completion_submitted_at
     )) then
    raise exception 'Công việc phải được chấm điểm cho lần gửi hiện tại trước khi hoàn thành.' using errcode='22023';
  end if;
