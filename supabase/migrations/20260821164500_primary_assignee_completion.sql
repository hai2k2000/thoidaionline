...
begin
  ...
  if v_role_code='tbt_read_only'
     or (v_role_code='tong_bien_tap' and p_action not in ('comment','review')) then
  ...
  case p_action
    ...
    when 'complete_assigned' then
      v_allowed := v_task.task_type='assigned'
        and v_task.assignee_id=p_actor_id
        and v_task.status in ('new','in_progress','blocked','waiting','rejected');
    when 'review' then
      v_allowed := v_role_code='admin'
        or (
...
alter function public.api_assert_task_action(uuid,uuid,text) set search_path=public,pg_temp;
...
begin
  ...
  if v_before.assignee_id<>p_actor_id or v_before.task_type<>'assigned' or v_before.status not in ('new','in_progress','blocked','waiting','rejected') then
    raise exception 'Task action forbidden.' using errcode='42501';
  end if;
  update public.tasks set status='pending_review',completion_submitted_at=v_now,updated_at=v_now where id=p_task_id returning * into v_after;
  insert into public.task_status_events(task_id,from_status,to_status,actor_id) values(p_task_id,v_before.status,'pending_review',p_actor_id);
  insert into public.audit_logs(actor_id,module,entity_type,entity_id,action,old_data,new_data) values(p_actor_id,'task','tasks',p_task_id,'submit',jsonb_build_object('status',v_before.status),jsonb_build_object('status','pending_review','completion_submitted_at',v_now));
  return v_after;
end
$function$;
