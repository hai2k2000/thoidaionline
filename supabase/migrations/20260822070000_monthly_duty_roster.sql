...
declare
  ...
  v_month_end date := (date_trunc('month',p_duty_month)+interval '1 month-1 day')::date;
  v_day jsonb;
  v_assignment jsonb;
  v_date date;
  v_position text;
...
begin
  ...
  from public.departments d where d.id=p_department_id and d.active=true;
  if not found then raise exception 'invalid duty department' using errcode='22023'; end if;
  ...
  for v_day in select value from jsonb_array_elements(p_days) loop
    ...
    for v_assignment in select value from jsonb_array_elements(v_day->'assignments') loop
      ...
      if not found or v_assignee_department is distinct from p_department_id
         or (v_position='Biên tập và xuất bản' and lower(coalesce(v_assignee_job,'')) not in ('tong_bien_tap','pho_tong_bien_tap'))
         or (v_position<>'Biên tập và xuất bản' and lower(coalesce(v_assignee_job,'')) not in ('phong_vien','truong_phong')) then
  ...
  loop
    ...
    if exists(select 1 from pg_temp.requested_duty r where r.due_date=v_existing.due_date and r.duty_position=v_existing.duty_position) then
      ...
      if v_existing.department_id=p_department_id and v_existing.reviewer_id=p_reviewer_id and v_existing.assignee_id=v_assignee then
        ...
        update public.tasks set department_id=p_department_id,reviewer_id=p_reviewer_id,
          assignee_id=v_assignee,owner_id=v_assignee,updated_at=now()
  ...
  for v_date,v_position,v_assignee in select due_date,duty_position,assignee_id from pg_temp.requested_duty order by due_date,duty_position loop
    ...
    values('Trực '||v_position||' ngày '||v_date,'Lịch trực tháng '||to_char(v_month,'YYYY-MM')||' · Vị trí: '||v_position,
      p_department_id,v_assignee,v_assignee,p_reviewer_id,p_actor,'individual',v_date,'17:00','new',0,'ad_hoc',false,
