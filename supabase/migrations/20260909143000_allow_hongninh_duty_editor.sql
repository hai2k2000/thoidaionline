...
-- September's approved roster includes Hồng Ninh in the editor rotation.
...
begin
  if exists (
    ...
    where t.id=new.task_id and t.task_category='duty'
  ) then
...
begin
  ...
  for v_day in select value from jsonb_array_elements(p_days) loop
    if jsonb_typeof(v_day)<>'object' or jsonb_typeof(v_day->'assignments')<>'array'
       or jsonb_array_length(v_day->'assignments') not between 1 and 4 then
      raise exception 'each duty day requires one to four positions' using errcode='22023';
    end if;
    ...
    for v_assignment in select value from jsonb_array_elements(v_day->'assignments') loop
      ...
      select jt.code,su.department_id,su.username into v_assignee_job,v_assignee_department,v_assignee_username
      ...
      if not found
         ...
         or (v_position in ('Biên tập bước 2','Biên tập bước 1') and lower(coalesce(v_assignee_username,'')) not in ('leson','triduong','thihung','hongninh'))
         or (v_position='Phóng viên' and (v_assignee_department is distinct from p_department_id or exists(select 1 from public.departments d where d.id=v_assignee_department and d.code='general') or lower(coalesce(v_assignee_job,'')) <> 'phong_vien' or lower(coalesce(v_assignee_username,'')) in ('thuphuong','thithuy','ngocanh','minhduc','ducanh','bachduong'))) then
        if v_position='Phóng viên' and exists(select 1 from public.departments d where d.id=v_assignee_department and d.code='general') then
...
declare
  ...
  v_result jsonb;
begin
  select id into strict v_actor from public.staff_users where username='admin' and active=true;
  select id into strict v_department from public.departments where code='editorial' and active=true;
  select id into strict v_reviewer from public.staff_users where username='quangthien' and active=true;

  with roster(day_no,publisher,editor,reporter) as (values
    (1,'dinhhai','triduong','hongninh'),
    (2,'dinhhai','triduong','thily'),
    (3,'dinhhai','leson','maianh'),
    (4,'quangthien','leson','thidoan'),
    (5,'quangthien','triduong','hongninh'),
    (6,'quangthien','hongninh','thily'),
    (7,'dinhhai','leson','maianh'),
    (8,'dinhhai','leson','thily'),
    (9,'dinhhai','hongninh','thidoan'),
    (10,'quangthien','hongninh','maianh'),
    (11,'quangthien','leson','thidoan'),
    (12,'quangthien','leson','thily'),
    (13,'dinhhai','hongninh','maianh'),
    (14,'dinhhai','hongninh','thidoan'),
    (15,'dinhhai','leson','maianh'),
    (16,'quangthien','leson','thily'),
    (17,'quangthien','hongninh','thidoan'),
    (18,'quangthien','hongninh','thidoan'),
    (19,'dinhhai','leson','thidoan'),
    (20,'dinhhai','leson','maianh'),
    (21,'dinhhai','hongninh','thidoan'),
    (22,'quangthien','hongninh','maianh'),
    (23,'quangthien','leson','thily'),
    (24,'quangthien','leson','thily'),
    (25,'dinhhai','hongninh','maianh'),
    (26,'dinhhai','hongninh','thily'),
    (27,'dinhhai','leson','thily'),
    (28,'quangthien','leson','maianh'),
    (29,'quangthien','hongninh','thidoan'),
    (30,'quangthien','hongninh','thily')
  ), resolved as (
    select r.day_no, p.id publisher_id, e.id editor_id, f.id reporter_id
    from roster r
    join public.staff_users p on p.username=r.publisher and p.active=true
    join public.staff_users e on e.username=r.editor and e.active=true
    join public.staff_users f on f.username=r.reporter and f.active=true
  )
  select jsonb_agg(jsonb_build_object(
    'date',make_date(2026,9,day_no),
    'assignments',jsonb_build_array(
      jsonb_build_object('position','Biên tập và xuất bản','assigneeId',publisher_id),
      jsonb_build_object('position','Biên tập bước 2','assigneeId',editor_id),
      jsonb_build_object('position','Biên tập bước 1','assigneeId',editor_id),
      jsonb_build_object('position','Phóng viên','assigneeId',reporter_id)
    )
  ) order by day_no) into v_days from resolved;

  if jsonb_array_length(v_days) <> 30 then
    raise exception 'September duty roster did not resolve all 30 days';
  end if;

  select public.api_save_monthly_duty_roster(
    v_actor,date '2026-09-01',v_department,v_reviewer,v_days
  ) into v_result;

  if coalesce((v_result->>'created')::int,0)
     + coalesce((v_result->>'updated')::int,0)
     + coalesce((v_result->>'unchanged')::int,0) <> 120 then
    raise exception 'September duty roster save was incomplete: %',v_result;
  end if;
end
$seed$;
