...
-- Convert the legacy four-position roster to Xuất bản, Biên tập, Phóng viên.
alter table public.tasks drop constraint if exists tasks_duty_position_check;

-- Keep the second editor step as canonical when both legacy rows exist.
create temporary table pg_temp.redundant_duty_editor on commit drop as
select id,status from (
  select id,status,row_number() over (
    partition by due_date
    order by case when duty_position='Biên tập bước 2' then 0 else 1 end,id
  ) rn
  from public.tasks
  where task_category='duty'
    and duty_position in ('Biên tập bước 1','Biên tập bước 2')
    and status <> 'cancelled'
) ranked where rn>1;

insert into public.task_status_events(task_id,from_status,to_status,reason,actor_id)
select id,status,'cancelled','Gộp vị trí Biên tập bước 1/2',
  (select id from public.staff_users where username='admin' and active=true limit 1)
from pg_temp.redundant_duty_editor;

update public.tasks t set status='cancelled', cancelled_at=now(),
  cancelled_by=(select id from public.staff_users where username='admin' and active=true limit 1),
  cancel_reason='Gộp vị trí Biên tập bước 1/2', updated_at=now()
from pg_temp.redundant_duty_editor r where t.id=r.id;

update public.tasks set duty_position='Xuất bản'
where task_category='duty' and duty_position='Biên tập và xuất bản';
update public.tasks set duty_position='Biên tập'
where task_category='duty' and duty_position in ('Biên tập bước 1','Biên tập bước 2');

alter table public.tasks add constraint tasks_duty_position_check check (task_category <> 'duty' or duty_position is null or duty_position in ('Xuất bản','Biên tập','Phóng viên'));
...
begin
  ...
  for v_day in select value from jsonb_array_elements(p_days) loop
    if jsonb_typeof(v_day)<>'object' or jsonb_typeof(v_day->'assignments')<>'array'
       or jsonb_array_length(v_day->'assignments') not between 1 and 3 then
      raise exception 'each duty day requires one to three positions' using errcode='22023';
    end if;
    ...
    for v_assignment in select value from jsonb_array_elements(v_day->'assignments') loop
      ...
      if v_position not in ('Xuất bản','Biên tập','Phóng viên') then
      ...
      if not found
         or (v_position='Xuất bản' and lower(coalesce(v_assignee_job,'')) not in ('tong_bien_tap','pho_tong_bien_tap'))
         or (v_position='Biên tập' and lower(coalesce(v_assignee_username,'')) not in ('leson','triduong','thihung','hongninh'))
